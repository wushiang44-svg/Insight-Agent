...
def _screen_item_llm(product_category: str, item: CollectedItem, llm: DeepSeekClient) -> ScreeningResult:
    system = (
        ...
        "IMPORTANT: `sentiment` must reflect the commenter's OWN stance, not the topic "
        "being discussed. If the text is disputing, debunking, or arguing against a "
        "problem someone else raised -- rather than the author reporting their own "
        "experience -- set `sentiment` to the commenter's own position (typically neutral "
        "or positive when defending the product), not the sentiment of the claim being "
        "disputed. This includes IMPLICIT disputes, not just explicit disagreement -- e.g. "
        "a short reply asserting that visible damage \"looks old\" or already existed is "
        "disputing that the product caused it. A genuine firsthand complaint stays negative even when phrased with a "
        "hedge, caveat, or skeptical-sounding wording -- only adjust `sentiment` for this "
        "reason when the text is actually arguing against someone else's claim, not "
        "reporting the author's own experience. This does not change which `insight_type` "
